...
先看原图上半部的闭环：模型优化把经验内化，Harness engineering 把能力外置，Agent 在环境反馈中持续运行；再看下半部时间轴，研究重心如何从模型训练扩展到外部运行时。密集的方法名用于定位版图，不必逐个记忆。

![论文原图 Figure 1：长程 Agent 研究版图与模型优化、Harness engineering 的双向关系](assets/long-horizon-agents-survey/paper-fig01-landscape.png)

*论文原图 Figure 1，来源：[Towards Long-Horizon Agents: A Survey](https://www.preprints.org/manuscript/202607.1328)。它支持综述对“内化模型能力 + 外置 Harness 能力”两条研究路线的组织方式，也显示作者如何把近年工作放进同一时间轴；它是文献分类图，不是控制变量实验，不能据此量化模型与 Harness 各自贡献。*
...
读原图时把左侧的 H1/H2/H3 与 C1/C2/C3 一一配对，再看右侧阶梯是否真的表示“同一能力越做越强”，还是已经从单窗口推理切换到跨窗口状态、再切换到跨任务学习。

![论文原图 Figure 3：长程任务的 H1—H3 层级及对应的 C1—C3 能力](assets/long-horizon-agents-survey/paper-fig03-horizon-levels.png)

*论文原图 Figure 3，来源：[Towards Long-Horizon Agents: A Survey](https://www.preprints.org/manuscript/202607.1328)。它清楚呈现三种依赖范围和三类能力的对应关系，支持本文后续按层拆解；它仍是作者提出的概念框架，尤其 H3 已进入跨任务经验积累，不能把阶梯外观当成严格嵌套或经验定律。*
...
先沿原图的山路从左到右看：Prompt engineering 控制单次行为，Context engineering 控制可见信息，Runtime Harness 则开始控制循环、反馈和恢复。三个阶段并非互相取代，而是控制对象逐步外扩。

![论文原图 Figure 6：从 Prompt engineering、Context engineering 到 Runtime Harness 的演进](assets/long-horizon-agents-survey/paper-fig06-evolution.png)

*论文原图 Figure 6，来源：[Towards Long-Horizon Agents: A Survey](https://www.preprints.org/manuscript/202607.1328)。它支持“控制面从响应级扩展到上下文级、再到轨迹级”的综述叙事；时间分段是作者对领域演进的综合判断，不是逐项因果验证，也不意味着 Prompt 或 Context engineering 已经过时。*

![本文重绘：控制面从 Prompt 迁移到 Runtime Harness](assets/long-horizon-agents-survey/01-control-surface-shift.svg)
...
读这张总览时先看六个彩色区域各自控制什么，再沿底部 `Set Goal → Complete & Deliver` 的轨迹检查它们何时介入。右侧 Hooks & Middleware 单独竖排，正好提醒我们它是跨阶段政策层，不只是另一个功能插件。

![论文原图 Figure 7：Long-Horizon Agent Harness 的六类系统责任与任务轨迹](assets/long-horizon-agents-survey/paper-fig07-harness-overview.png)

*论文原图 Figure 7，来源：[Towards Long-Horizon Agents: A Survey](https://www.preprints.org/manuscript/202607.1328)。它直接支持 Loop、Context、Tools、Orchestration、Hooks、Verification 六类责任的分类与覆盖范围；分类来自文献综合，模块边界会在真实系统中重叠，图本身也不能证明“六类齐全”就足以获得可靠长程能力。*
...
![本文重绘：模型与 Harness 的共演化闭环](assets/long-horizon-agents-survey/02-model-harness-coevolution.svg)
...
读原图时先确认纵轴是“人类专家完成任务所需时长”对应的 50% 成功率 horizon，并注意它使用对数刻度；右侧柱状图是同一指标的模型切片，不是模型实际连续运行时长。

![论文原图 Figure 4：前沿模型 50% 任务时间跨度的历史趋势与代表模型比较](assets/long-horizon-agents-survey/paper-fig04-time-horizon.png)

*论文原图 Figure 4，来源：[Towards Long-Horizon Agents: A Survey](https://www.preprints.org/manuscript/202607.1328)，数据叙事引用 METR time-horizon 研究。它支持在给定任务集和 Harness 下，50% 成功率对应的人类任务时长快速上升；它不证明模型能在现实开放环境中无监督稳定运行同样久，也不能把人类耗时直接当成通用智能刻度。*
...
![本文重绘：论文事实、本文推论与开放问题](assets/long-horizon-agents-survey/03-evidence-boundary.svg)
...
1. Dong et al., [Towards Long-Horizon Agents: A Survey](https://openreview.net/pdf?id=HyhfhlbWGh), OpenReview, 2026；[Preprints 手稿页（CC BY 4.0）](https://www.preprints.org/manuscript/202607.1328)。配套资料库：[Awesome Long-Horizon Agents](https://github.com/RUC-NLPIR/Awesome-Long-Horizon-Agents)。

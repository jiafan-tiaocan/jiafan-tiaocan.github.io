...
description: "从真实业务系统出发，区分业务策略实践与专家协作两条经验形成闭环，给出 Mem-OS 的对象模型、受控写入、上下文编译、经验晋升、治理与评测方法。"
tags:
  - Agent
  - Mem-OS
  - Agent-OS
...
>
> 对神童而言，这个内核必须长期跑通两条不同的经验链路：一条从**业务策略实践**中学习“什么做法在什么条件下有效”，另一条从**与专家共同完成任务**的过程里学习“专家如何判断、为什么修改、何时允许例外”。只保存最终结果，或者只做通用对话总结，都会丢掉真正能形成壁垒的部分。
>
> 它最重要的边界可以浓缩成一句话：**模型负责提出语义，确定性系统负责承诺状态。**

本文以《Mem-OS：真正构成 Agent 壁垒的知识与经验系统》PDF 中的源码研究和业务经验为底稿，但不复述其中大量产品目录、代码片段与业务名词，而是将它们抽象为三类普遍场景：受监管内容生产、个人工作助理和持续运营复盘。外部论证主要采用官方文档、官方仓库和论文原文。
...
![对话、Tool、环境事件和人工反馈先进入不可变来源层，再由 Mem-OS 的写入、读取和生长内核编译为 Agent Core 上下文](assets/mem-os-agentos-position.svg)

*图 1：Mem-OS 位于业务事件与 Agent Core 之间，治理平面约束知识如何写入、激活和演进。自绘图；依据神童受控写入实践、本文对象边界及 LangGraph Memory / MemOS 的公开系统抽象，公开版不包含内部路径和数据。*
...
![知识提案经过准入、冲突与审核后成为有效记忆，之后可以被新版本替代或因过期和遗忘而退役](assets/mem-os-record-lifecycle.svg)

*图 2：知识不是直接写入，而是在 Proposed、Draft、Disputed、Active、Superseded 与 Retired 之间受控迁移。自绘图；依据神童写前校验、版本哈希、备份回滚和夜间写入提案机制抽象。*
...
![原始 Source 与 Episode 位于底层，从单次来源提取的记忆位于中层，跨经历归并和验证后的知识位于顶层](assets/mem-os-knowledge-layers.svg)

*图 3：原始证据、提取记忆与编译知识必须分层，任何上层结论都能回到来源。自绘图；依据本文 Source / Episode / Procedure 对象模型及非破坏性归并原则。*
...
神童的 Mem-OS 不能把“经验”理解成一种统一的会话摘要。真正有复利价值的经验至少来自两条链路：

| 经验链路 | 学习对象 | 最强证据 | 主要产物 | 最大误区 |
|---|---|---|---|---|
| 业务策略实践 | 什么策略在什么条件下有效或失效 | 任务上下文、版本化行动、业务 Outcome、对照与反例 | Episode、Belief、候选 Procedure | 把一次指标上涨直接写成方法 |
| 专家协作任务 | 专家怎样判断、为何修改、何时例外 | Agent 原提案、专家 Diff、判断理由、证据要求与最终决策 | Claim、Preference、Policy、候选 Procedure | 只保存最终答案，丢掉专家判断过程 |

两条链路的共同目标都是形成可复用经验，但它们提供的监督信号不同：业务实践给出的是 **Outcome Supervision**，专家协作给出的是 **Reasoning / Decision Supervision**。前者告诉系统“后来发生了什么”，后者告诉系统“为什么此刻应该这样做”。任何一条单独存在，都不足以支撑稳健的经验晋升。

#### 5.1.1 从业务策略实践中总结经验

业务策略实践的最小单位不是一轮对话，而是一个完整的 Strategy Episode：

```yaml
experience_source: business_practice
task_contract: 目标、约束与成功口径
context_snapshot: 用户 / 产品 / 渠道 / 时间 / 环境
strategy:
  selected: strategy_v3
  alternatives_considered: [strategy_v2, manual_fallback]
execution:
  trace_ref: run_123
  human_interventions: [step_8_approved]
outcome:
  observation_window: 7d
  business_metrics: [...]
  quality_and_risk: [...]
comparison: baseline_or_matched_cases
confounders: 流量、人群、价格、季节性与外部变更
```

这里必须保留五类信息：

1. **当时面对什么问题**：任务目标、限制条件和实体范围决定经验能否迁移；
2. **为什么选择这条策略**：只保存执行动作，会丢失策略选择和备选方案；
3. **实际怎样执行**：Tool Trace、参数、人工介入和中途变化决定结果能否归因；
4. **结果在什么窗口出现**：即时完成、质量反馈和业务指标可能有不同延迟；
5. **有哪些反例与混杂因素**：流量、人群或政策变化可能制造虚假的策略收益。

一次成功默认只形成 Episode；有合理机制但证据不足时形成 Belief 或候选 Procedure；只有在重复 Case、对照切片、重放或受控实验中保持效果，才有资格晋升为 active Procedure。业务结果再好，也不能自动把一种做法升级成 Policy，因为“有效”与“必须遵守”是两种完全不同的真值。

#### 5.1.2 从专家协作任务中总结经验

专家协作的价值也不在最终交付物本身。真正稀缺的知识通常藏在协作过程的 Diff 里：

- Agent 最初提出了什么方案、引用了什么证据、在哪些地方不确定；
- 专家选择接受、修改、拒绝还是重排，改动发生在哪个决策点；
- 专家为什么这样改，要求补看什么证据，担心什么风险；
- 这个判断是通用规则、领域事实、个人偏好，还是当前 Case 的例外；
- 修改后结果如何，后来是否被同一专家或其他专家再次确认。

因此，一次专家协作至少应形成：

```yaml
experience_source: expert_collaboration
agent_proposal_ref: artifact_123
expert_action: modify       # accept / modify / reject / reorder
decision_diff: diff_456
rationale: 为什么原方案不足、采用了什么判断依据
requested_evidence: [...]
scope: project
exceptions: [...]
authority: domain_expert
confidence: expert_confirmed
final_outcome_ref: artifact_789
```

系统不能把所有专家改动都保存成 `Preference`。一个明确、可核证的领域事实应成为 Claim；经授权发布的强制要求才是 Policy；稳定重复的判断路径可以成为候选 Procedure；只代表某位专家工作风格的内容才是 Preference；尚未证实的经验判断仍应停留在 Belief。

同样，`expert_confirmed` 不代表可以绕过所有验证。专家有权确认自己领域内的事实、规则和当前任务决策，但一个准备跨项目复用的 Procedure 仍需经过新 Case、near-miss 和反例评测。否则系统只是把“模型自信”换成了“专家权威”，并没有解决错误迁移。

#### 5.1.3 两条链路进入同一个经验编译器

![左侧业务实践从任务、策略、行动和 Outcome 提炼经验，右侧专家协作从 Agent 提案、专家修改和判断理由提炼经验，两路证据在统一编译器中分类、验证和提交](assets/mem-os-dual-experience-loops.svg)

*图 4：神童的双经验闭环。业务实践提供结果监督，专家协作提供判断监督；实践异常进入专家诊断，专家假设再回到真实业务验证。自绘图；依据神童两类经验沉淀需求，公开版仅保留脱敏后的系统契约，不展示内部会话、人员或业务指标。*

两条链路不应建设成两套互不相干的知识库，而应共享一个 Experience Compiler：

```text
Strategy Episode / Expert Collaboration Episode
→ Evidence Bundle
→ 类型识别与 Scope 限定
→ Memory Proposal
→ 去重、冲突、反例与隐私检查
→ 重放 / Holdout / 专家或 Policy 审核
→ Commit 为 Claim / Preference / Policy / Procedure / Belief
→ 在下一次任务中激活并记录 Outcome
```

它们还应互相校验：业务实践中出现无法解释的异常，进入专家复盘；专家提出的新判断，回到真实策略实践验证。这样形成的不是“Agent 自动总结自己”，而是一个业务结果、专家判断和系统证据相互制约的学习闭环。

Mem-OS 负责保留、组织和编译这些经验；当经验要进一步改变 Skill、Tool、Prompt 或组织策略时，就进入更严格的评测与发布系统。两者的边界可参见 [[Agent Self-Evolution：从反馈闭环到可验证的系统进化]]。
...
![业务事件和原始材料进入受治理的写入内核，经知识状态层保存后由读取内核编译为 Agent 上下文，后台归并和结果评测持续修订知识](assets/mem-os-reference-architecture.svg)

*图 5：Mem-OS 通用参考架构。自绘图；写入事务来自神童实践，Context Compiler 与后台归并参考本文公开资料，Outcome Ledger 则承接业务策略实践和专家协作两条经验链路。*

架构中的关键组件并不要求一次全部建设，也不绑定某个数据库：

1. **Source & Episode Registry**：保存对话、文档、Tool 结果、Trace、反馈和 Artifact 的不可变引用；业务 Strategy Episode 额外绑定 Outcome，专家 Collaboration Episode 额外绑定 Proposal、Diff 与 Rationale。
...
6. **Experience Consolidator & Procedure Compiler**：让业务结果监督与专家判断监督进入统一提案协议，后台执行去重、压缩、时间冲突处理、候选方法提炼和 Scope 晋升。
...
record_outcome(strategy_outcome_event)
record_expert_review(expert_review_event)
...
| 阶段 | 关键问题 | 推荐指标 |
|---|---|---|
| Write | 该记的是否记了，不该记的是否挡住 | Write Precision / Recall、Outcome Linkage、Expert Rationale Coverage、越权写入率 |
...
| Learn | 两条经验链路是否产生可迁移方法 | Strategy Transfer、Expert Correction Reuse、Counterexample Pass Rate |
...
11. 一次专家修改能否保留原提案、Diff 和理由，并在相似新任务中正确复用而非机械照搬；
12. 一条业务策略换到不同用户、产品或渠道后是否保持限域，是否能识别反例和混杂因素。
...
1. 定义 `Source / Episode / Claim / Preference / Policy / Procedure` 的最小 Schema，并区分 `business_practice` 与 `expert_collaboration` 两类 Episode；
2. 所有有效记录带 `scope / status / evidence / time / revision`；
3. 建立 Proposal → Admission → Commit，支持 CAS、幂等、审计和回滚；
4. 把索引降为可重建派生物，为每次任务固定知识快照；
5. 记录 `StrategySelected / OutcomeObserved / ExpertCorrected / ExpertRationaleCaptured / MemoryActivated / Applied / Contradicted` 事件；
...
6. 分别建立策略实践 Reviewer 与专家协作 Reviewer：前者检查 Outcome、对照与混杂因素，后者检查 Proposal、Diff、Rationale 与 Scope；
7. 将 Memory 使用事件与 Action、Outcome 关联，开始测收益和负迁移。

### P2：让经验稳定地变成组织能力

1. 从重复成功与失败的 Strategy Episode，以及重复出现的专家判断 Diff 中提炼候选 Procedure；
...
5. **双经验闭环**：既能从策略实践的 Outcome 学习，也能从专家协作的 Diff 与 Rationale 学习，并让两类证据互相验证；
6. **程序晋升机制**：能把多次经历变成经过验证、可回滚的 Procedure 或 Skill；
7. **评测与结果数据**：积累了真实 near-miss、冲突、过期、负迁移和业务结果 Case；
8. **治理信任**：用户能看到、纠正、限制和删除记忆，组织能审计每一次行为变化。
...
> **经验从两处生长：在业务策略实践中学习何时有效，在专家协作中学习为何这样判断；两者都必须经过编译、验证和发布。**  

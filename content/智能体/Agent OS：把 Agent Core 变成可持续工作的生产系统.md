...
description: 从两类真实业务系统出发，区分 Agent Core 的机制与 Agent OS 的策略，并拆解动态工作拓扑如何被编译为受治理、可恢复、可观测的生产执行。
tags:
  - Agent-OS
  - Agent-Core
  - 智能体
...
> [!tip] 阅读路线
> - 只想建立直觉：读第 0～3 节，先理解 Core 与 OS 的边界以及六个平面。
> - 关心业务形态：读第 4～5 节，对比持续运营与素材生产为何需要不同的 Agent 图。
> - 准备落地系统：读第 6～11 节，重点看状态机、耐久执行、动态拓扑、完成证明和全链路观测。
> - 负责版本演进：读第 12～14 节，用评测、发布门禁和 30 个问题检查系统是否仍停留在 Demo。
...
运行平面还必须区分“进程存在”和“可以接收真实流量”。一个可操作的就绪链至少是：
...
→ tool_dependencies_ready
→ ready_for_traffic
```

只有 PID 或心跳，最多证明容器还在；它不能证明模型连接、队列、Tool 依赖和权限门已经可用。就绪状态应成为结构化数据，并与流量准入和自动摘除直接关联。

常驻 Runtime 也不是越多越好。Agent 的逻辑身份可以持续增长，热 Runtime 必须受内存、模型并发、会话重建成本和租户优先级约束。运行平面需要保留会话局部性，同时用有界池、驱逐策略和 Checkpoint 让物理承载可替换。
...
固定 DAG 容易在创意返工时失效；完全自由委派又容易无限循环。更准确的边界是：**Agent 提议工作拓扑，OS 把拓扑编译为受治理的执行契约。**

> [!note] 2026-07-23 补记与证据边界
> 一次脱敏后的生产复盘显示：面对原本按串行方式理解的创作任务，Agent 在运行时识别出独立分支，并通过既有委派、队列和 Worker 原语组合出了并行路径。这个信号说明动态拓扑的方向成立，但由于没有可公开复现的完整 Trace，本文不声称具体效率提升，也不把尚未内核化的 TaskGroup、写集合检测和自动 Join 描述为现有事实。公开版已移除内部项目、基础设施和参与者信息。

![Agent 提议拓扑，Agent OS 编译并执行](assets/agent-os-production/15-dynamic-topology-compiler.svg)

两边的责任不能混淆：

| 阶段 | Agent 可以决定 | Agent OS 必须决定 |
|---|---|---|
| 拆解 | 建议节点、依赖、专业角色和目标 | 节点是否形成合法 Task，目标是否符合业务合同 |
| 编排 | 建议串行、并行、回访或委派 | 输入是否就绪、依赖是否无环、能力是否允许 |
| 准入 | 给出并行理由和预期收益 | 写集合是否冲突、预算与并发度是否允许 |
| 汇合 | 建议如何比较或归纳结果 | Join Policy、结果 Schema、失败与超时语义 |
| 交付 | 解释结果并建议下一步 | 证据是否完整、Root Task 能否进入终态 |
...
- 并行节点必须输入就绪、相互无依赖、写集合不冲突、结果可以汇合；
- 调度器只运行 Ready Set，并受 `max_parallelism`、租户配额与外部 Tool 容量约束；
- 人工补参、选择、审核和验收是正式状态，不是普通 Tool Result。

业务工作图可以再次回到“编剧”或“设计”等同类角色，但每次回访都要创建新的 Task 节点。这样创作路径可以循环，执行事实仍保持为可审计、可重放的有向无环历史。
...
class AgentTask:
    ...
    def scheduling_key(self) -> str:
        if self.operation == "project_asset_write":
            return f"project:{self.project_id}:asset:{self.asset_id}"
        if self.operation == "global_model_call":
            return f"tenant:{self.tenant_id}:model:{self.model_id}"
        return f"project:{self.project_id}:agent:{self.agent_id}"
```
...
- `scheduling_key` 不是队列命名习惯，而是把业务身份和操作类型变成可执行的并发契约。

默认语义可以从一张小矩阵开始：

| 业务关系 | 默认执行语义 |
|---|---|
| 同项目、同专业 Agent | 串行，保持会话与局部状态顺序 |
| 同项目、不同专业 Agent | 允许并行，但必须继续检查共享资产写冲突 |
| 不同项目 | 允许并行，但受租户预算、模型容量和全局并发限制 |
| 任意任务写同一资产 | 收敛到资产资源键，强制串行或显式版本合并 |

随着系统演进，并发键可以从“项目 + Agent”细化为“项目 + 资产”，也可以提升为“租户 + 模型”。关键不是键长什么样，而是谁的竞争会破坏正确性。
...
### 5.4 Fan-out 需要 TaskGroup、Ready Set 与 Join

连续多次单目标委派可以偶然形成并行，但那还不是完整的 fan-out。要把动态拓扑变成可解释、可恢复的一等能力，需要显式 TaskGroup：
...
class TaskGroup:
    ...
    parent_task_id: str
    join_policy: Literal[
        "all_success", "best_effort", "first_success", "quorum"
    ]
    max_parallelism: int
    deadline_at: datetime | None
    reducer_agent_id: str | None
...
1. Agent 提交 fan-out 提案，说明子目标、依赖、输入、预期产物和汇合意图；
2. OS 校验目标 Agent、委派深度、能力边界、总预算、写集合与结果契约；
3. Scheduler 计算 Ready Set，在 `max_parallelism` 和资源配额内领取节点；
4. Join Controller 根据策略汇合，并在需要时把结构化结果交给 Reducer Agent。

父 Task 在等待单个子任务时进入 `WAITING_CHILD`，等待一组分支时进入 `WAITING_GROUP`。子任务的有效能力应取父级授权、目标 AgentSpec 和本 Task 作用域的交集；委派只能缩小权限，不能让能力沿调用链意外升级。

### 5.5 AIGC Tool 是运行平面的一级资源
...
### 5.7 Root Task 把动态内部拓扑压缩成连续的用户工作

内部可以产生几十个 Child Task、重试分支和人工等待，用户仍应只面对一项稳定工作。Root Task 是这项工作的统一所有权边界：

![Root Task 维持用户工作连续性](assets/agent-os-production/16-root-task-continuity.svg)

- 所有后代事件归入 Root Event Channel，但每个消费者拥有独立 Cursor；
- 浏览器或 SSE 断线只意味着消费者离线，不定义任务死亡；
- 用户重新连接时按 Cursor 续传，不与监控、审计等其他消费者抢事件；
- Root 聚合预算、成本、风险、未完成后代和最终完成证据；
- 用户取消的是整项工作，不是当前 HTTP 请求。
...
每一层都要共享取消原因或 Cancellation Token，并在领取前、执行循环中和真实副作用前检查。只关闭连接而不终止后台任务会产生幽灵执行；只杀 Worker 而不取消外部生成任务又可能继续计费和回调。
...
Event、Log 和 Trace 不能混成同一种流水账：

| 类型 | 主要服务对象 | 回答的问题 |
|---|---|---|
| Event | 用户、调用方与状态机 | 发生了什么，接下来应等待还是继续 |
| Log | 开发与运维 | 在哪里失败，为什么失败 |
| Trace / Span | 系统治理与性能分析 | 一项工作如何跨 Agent、Tool、Worker 和基础设施传播 |

三者可以使用不同的保留期、采样率和脱敏策略，但必须共享关联键。建议每个事件和 Span 都能关联：
...
- 将运行时拓扑提案升级为可审计对象，记录节点、依赖、并行理由和汇合意图；
- 引入 TaskGroup、Ready Set、Join Policy、Reducer 与写集合冲突检测；
- 用项目/Agent/资产等业务身份生成并发键，并设置委派预算与循环检测；
- 人工等待跨天持久化；
- Root Event Cursor、级联取消、AIGC Tool Pool、产物血缘与可恢复编辑 Checkpoint。

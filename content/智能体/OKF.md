...
![OKF、本体、知识管理、知识检索、知识图谱和记忆系统的协作关系](OKF：开放知识格式完整解读.assets/05-knowledge-relationships.svg)

图 5　外框表示治理职责，虚线表示可选的语义约束与映射，实线表示图中标明的读取或写入。OKF 和图谱可以分别使用；采用本体、互相映射，都需要具体实现。记忆也可以直接通过状态读取进入当前任务，图中只突出经验检索这条路径。治理职责可以跨越多个系统，外框不表示软件模块的严格包含关系。

...
把第 1 章的同月交易具体设为 9 月线上交易，将该例现有口径简称为“政策 P1”。下面直接看这笔 `100 + 200 − 20 = 280` 怎样串起六种职责；P1 只是图中的教学标签，不是 OKF 标准字段。

![教学案例：政策 P1、净收入文件、退款 R 与订单 B、检索依据和 280 元任务记忆怎样协作](OKF：开放知识格式完整解读.assets/05-knowledge-relationships.svg)

图 5　每个框都给出同一案例中的对象或动作。外框表示贯穿其间的治理职责；虚线表示可选语义约束，图谱框内箭头是业务关系，框间实线表示读取、写入或召回。图中政策是简写，仍适用第 1 章 paid 订单、confirmed 退款及关联渠道等条件；具体数据由 Agent 按计算约定读取，OKF 表定义不等于整张交易表。

从图上可直接区分：本体定义“退款关联订单”这种关系，图谱记录“R 关联 B”这条具体声明；OKF 交付带来源与复核信息的知识文件，检索为本轮选取它们；记忆保存“这次为什么得到 280”，下次仍要重查适用口径与数据。OKF 与图谱可以分别使用，记忆也可以直接通过状态读取进入任务，图中的组合不要求六套独立软件，也不代表经历能自动变成已批准政策。

...
这篇文章默认读者知道卷积、特征图、池化和 softmax，不要求先学过医学图像。

- 只想建立直觉：读“一个会失败的场景”“U 的两边各负责什么”和“实验究竟证明了什么”。
- 想实现原版：继续看 572→388 的尺寸变化、`copy and crop`、加权损失和 overlap-tile。
- 想理解扩散模型里的 U-Net：直接看“原始 U-Net 和扩散 U-Net 不是同一个任务”，但不要跳过证据边界。

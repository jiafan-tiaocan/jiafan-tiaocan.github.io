...
若要把这项工程选择放回完整论文谱系，可继续阅读：[YOLO 演进：从统一回归到实时检测工程体系](../算法基础/YOLO演进：从统一回归到实时检测工程体系.md)。该文把检测输出统一写成 $\mathcal{D}(I)=\{(b_i,c_i,s_i)\}$：本项目实际消费的是 Logo 对应的矩形 $b_i$、类别 $c_i$ 与筛选分数 $s_i$。YOLOv7 论文在 COCO 上验证的是实时检测架构、重参数化和训练期标签分配；它没有验证“保留整个 Logo 矩形图块更能维护品牌完整性”。后者来自项目对小字、描边、阴影和字标的验收标准，是超出论文证据的业务决策。

...
你记得的 **Bounding Box** 和 **YOLO** 都存在，但它们不是同一个层级的问题。

项目确实使用了 **YOLO V7**，用途是检测 Logo 的矩形区域；Logo 中常含小字和复杂字标，强行做透明抠图反而容易破坏品牌细节，因此项目有意保留整个图块。真正需要像素级边界的按钮，则采用 **GroundingDINO 先定位、SAM2 再分割**。开放形态的主视觉后来交给 **Qwen2.5-VL** 理解整张卡片并输出 BBox，最后再做区域抠图。前景移走后的空洞，不属于分割问题，而由 **FLUX-Fill、对象移除 LoRA 与 LaMa** 共同恢复。

所以更准确的项目总结是：

> **这不是“一种 YOLO 分割算法”，而是一条把语义识别、几何定位、像素分割、背景恢复和图层重建分别交给合适模块的混合工作流。**

## 阅读路线

- 只想确认项目用法：直接读“项目复盘：我们到底用了什么”。
- 想补齐概念：读“先把六个容易混淆的任务分开”和“技术演进”。
- 想重新实现：读“推荐生产架构”“代码骨架”和“评测门禁”。

> [!note] 证据边界
> - 项目发生在 2025 年；公开模型资料于 **2026-07-30** 复核，不能把后续版本能力倒写成当时已经使用。
...
“图像分割”在口语里经常包办上述所有任务，但工程上必须拆开。否则很容易出现三类误判：

1. 看到红框就说“完成了分割”；
2. 得到漂亮 mask 就以为“已经有可编辑模版”；
3. 擦除后背景自然，就以为前景定位一定准确。

## 2　技术演进：从固定类别的像素到开放语义的可执行契约

技术史不是一条模型榜单，而是旧输出遇到新需求后不断扩展契约的过程。

### 2.1 U-Net：让网络一次输出整块像素标签

分类网络只给整张图一个标签，滑窗像素分类又会重复计算。U-Net 用收缩路径获取大范围上下文，用扩张路径恢复空间分辨率，并把同尺度的编码器特征直接拼回解码器。

看图时最值得注意的不是 U 形轮廓，而是横向 `copy and crop`：深层特征回答“这是什么”，浅层高分辨率特征帮助回答“边界具体在哪里”。
...
这一范式给项目一个很实用的启发：**先定位，再在局部区域内精细分割**往往比直接对整图做开放类别分割更稳。项目后来采用的 GroundingDINO + SAM2，本质上也是把“找哪个区域”和“区域内哪些像素”拆成两个责任，只是类别从固定标签进一步变成了语言提示。

### 2.3 YOLOv7：项目要的是快速 BBox，不是全图像素 mask
...
原因不是“不会分割”，而是品牌资产的验收标准不同。Logo 常同时包含图形、小字、描边和阴影；像素级抠图一旦切坏一个笔画，损失比带入少量背景更严重。项目选择了一个很工程化的优先级：

> **对 Logo，品牌保真高于资产完全解耦。**

现代工具链里确实存在带 segmentation head 的 YOLO 变体，但这不能反推本项目使用了它。我们只能确认项目用了 YOLO V7 做 Logo BBox 检测。
...
- **Bounding Box** 是正确的术语；
- **YOLO V7** 也确实用过；
- 但 YOLO V7 只负责 Logo 的 BBox 检测；
- 主视觉后来用 Qwen2.5-VL 做全局语义定位；
- 按钮采用 GroundingDINO 定位、SAM2 分割；
- 文字由 OCR 重建；
- 背景由 FLUX-Fill、移除 LoRA 与 LaMa 修复；
- 最终由 Sketch 图层和真实编辑动作验收。

这套方案最值得总结的不是模型名单，而是责任分配：

> **先用全局语义决定“它是什么、该不该成为独立资产”，再用 BBox 决定“从哪里开始找”，用 mask / alpha 决定“边界到哪里”，用 Inpainting 决定“移走以后露出什么”，最后用编辑器决定“这个结果是否真的可用”。**

...
description: "回到 2023—2024 年我们把 ComfyUI 接入生产的真实场景，再以 v0.28.0 与 Frontend v1.45.21 源码复核它的计算图、增量执行、显存管理和生产边界。"
tags:
  - ComfyUI
  - AIGC
  - 图像生成
...
> [!note] 时间说明
> 这篇文章按我们深度使用 ComfyUI 的 **2023—2024 年实践期**归档，公开日期设为 2024 年 7 月 23 日。为了不靠旧印象解释今天的 ComfyUI，文中的底层原理和接口又使用 2026 年 7 月的稳定源码做了复核。实践发生时间与证据核验时间是两条刻意分开的时间线。

> [!abstract] 核心判断
> 我们当年选择 ComfyUI，不是因为拖拽节点比写 Python 更“低代码”，而是因为算法同学刚在画布上验证过的复杂生成流程，可以直接编译成 JSON 交给生产服务执行。研究原型和线上执行第一次共享了同一份可见制品。
>
> 从源码看，它也不只是 Stable Diffusion 的“节点版界面”，而是 **可视化编程前端、带类型的数据流中间表示、增量 DAG 执行器和异构模型运行时**的组合。不过它只解决单次生成任务内部的计算编排；持久队列、认证、幂等、跨进程恢复和多租户隔离仍应由外部生产系统负责。
...
## 一、先回到 2023—2024：为什么这张图当时很重要

当图像生成还只是“输入一句 Prompt，返回一张图”时，一个 Python 函数已经够用。但真实业务很快变成：接收用户图片、裁切与缩放、编码 latent、叠加 ControlNet 或 LoRA、控制重绘幅度、批量采样、筛选候选，再保存带元数据的结果。流程一复杂，Notebook 和脚本就暴露出同一个问题：**只有写它的人知道数据究竟怎样流动。**

更麻烦的是研发与生产之间的第二次翻译。算法同学在 Notebook 里调通之后，工程同学还要重新实现模型加载、条件组合、采样和后处理。参数会漏、顺序会变、显存策略会被重写，最后线上运行的已经不是最初验证的那条流程。

ComfyUI 当时真正击中我们的，是把这次翻译压缩成了一次工作流发布。以版权重绘链路为例，算法同学把完整方案留在画布里；接入端只需要绑定输入图片、重绘幅度、随机种子和候选数量，提交导出的 API Prompt，GPU Worker 就能执行同一张图。它不是消灭代码，而是给算法和工程提供了一种双方都看得见、机器又能直接执行的中间语言。

这也是理解全文最重要的起点：

> **ComfyUI 的核心价值不是“节点很多”，而是把生成算法从个人脚本变成可以检查、组合、发布和调用的执行图。**

> [!tip] 阅读路线
> - 想先建立直觉：读第三、四节，理解“计算图”与“一张图的四次变形”；
> - 想理解为什么交互快：读第五至八节，重点看双重 JSON、输出驱动和变化闭包；
> - 正在做生产接入：直接读第十至十五节，那里给出早期案例、执行包、API 与部署门禁；
> - 只想拿走判断：读下面的五行结论，再跳到第十八节。

### 1.1 先把结论交给你

| 读者最关心的问题 | 简短回答 | 文章中的证据 |
|---|---|---|
| 它究竟是什么？ | 面向生成模型的可视化数据流编程与执行引擎 | 画布会被编译为 API Prompt，再由服务端按图校验和调度 |
| 为什么修改局部后不用整图重跑？ | 执行器从目标输出反向展开，并复用输入签名未变化的节点 | `ExecutionList`、输入签名与输出缓存 |
| 为什么适合研发接生产？ | 探索画布和服务端执行图共源，外部服务只需注入受控参数 | `graphToPrompt`、`/prompt` 与我们早期的版权重绘接入 |
| 它不是什么？ | 不是 Temporal 一类可跨进程恢复的耐久业务工作流 | 队列、运行态和 History 主要保存在单进程内存中 |
| 上生产最该防什么？ | 插件供应链、版本漂移、重复提交和裸端口暴露 | 自定义节点是任意 Python；客户端 UUID 也不自动提供幂等 |

## 二、版本锚点与研读边界

本文的实践叙事属于 2023—2024 年；为确认这些经验在当前实现中的准确含义，原理与接口核验固定在 2026 年 7 月 23 日可获得的稳定组合，而不是不断变化的 `master`：
...
接下来可以沿着一次真实请求继续往下读：画布先变成什么 JSON，后端怎样决定执行顺序，为什么只重算变化分支，最后又怎样管理模型和显存。这样每个源码概念都会落回图 2 中的一个位置，而不是变成彼此孤立的实现细节。
...
## 十、早期生产接入：有效之处与欠下的工程债

第一节先讲了这段实践为什么重要，这里把它落到具体的接入契约。此前的《版权重绘：基于视觉反推与提示词工程的低版权风险生成方案》留下了一条很典型的生产路径：

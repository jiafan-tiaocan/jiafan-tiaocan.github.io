...
本文默认读者知道 Transformer 里的 token、Self-Attention、残差连接，也见过扩散前向公式。Attention 的 Q/K/V、mask、Multi-Head 与 Cross-Attention 可先看 [[论文解读：Attention Is All You Need]]；扩散的概率推导可先看 [[努力做一个可以让人记住的Diffusion推导]]。这里仅保留理解 DiT 必需的部分。

把两篇论文的符号对齐，原始 Transformer 的输入序列 $X$ 在 DiT 中变成 VAE latent 的 patch token；DiT block 的 Self-Attention 仍是 $Q=XW^Q,K=XW^K,V=XW^V$，只是 token 从词片换成了二维空间块。若后续文本条件 DiT 使用 Cross-Attention，则通常由图像 token 提供 Query、文本 token 提供 Key/Value；这与原始翻译 Decoder 读取 Encoder 的方向完全同构。

证据边界也必须分开：2017 年 Transformer 论文验证的是机器翻译与句法分析中的序列转导质量、训练并行性及部分组件变体，并没有验证图像生成；DiT 的 VAE latent、二维位置编码、扩散目标、时间条件、adaLN-Zero、classifier-free guidance 与反复去噪采样都属于后续系统实践，不能被算作原始 Transformer 论文的实验结论。
...
![本文重绘：DiT 在完整生成系统中的位置——VAE 负责压缩与解码，扩散过程负责加噪和反向更新，DiT 负责每一步的噪声与方差预测](assets/dit-system-map.svg)
...
这里的“卷积 U-Net”不能直接等同于 2015 年的医学分割网络。[[论文解读：U-Net: Convolutional Networks for Biomedical Image Segmentation]] 解决的是干净图像到像素标签的映射，用收缩—扩张路径和同尺度 `copy-and-crop` 拼接协调上下文与定位；扩散系统继承的是这套多尺度信息骨架，又把输入改成带噪 $z_t$，加入时间步与类别/文本条件，把输出改成噪声、score、velocity 或方差相关参数。原始 U-Net 论文没有验证生成、时间条件或 Attention，因此 DiT 的对照对象是**后来的扩散 U-Net**，不是把 2015 年分割论文原封不动搬进生成系统。

## 先看责任发生了什么变化

U-Net 和 DiT 的外部任务几乎一样：都要根据 $(z_t,t,y)$ 预测反向更新所需的信息。区别在于内部怎样让空间位置交换信息。

![本文重绘：U-Net 与 DiT 的责任对比——扩散外循环不变，空间建模从多尺度卷积路径换成固定分辨率 token 上的全局 Transformer](assets/dit-unet-vs-dit.svg)
...
先沿原图从左向右看三件事：带噪 latent 如何进入主干，时间与类别条件如何进入 block，以及 adaLN-Zero 与 Cross-Attention、in-context conditioning 的接线差异。

![论文原图 Figure 2：DiT 总体架构、adaLN-Zero block，以及另外两种条件注入方式](assets/dit-paper-fig02-architecture.png)

*论文原图 Figure 2，来源：[Peebles & Xie, Scalable Diffusion Models with Transformers](https://arxiv.org/abs/2212.09748)。左侧确认 DiT 仍处在 latent diffusion 的去噪位置；中间把条件转成 scale、shift 与残差 gate；右侧是对照结构。它支持“主干与条件接线被共同改写”的结构事实，但结构图本身不证明 adaLN-Zero 的质量更好，性能差异还要看后文消融。*
...
![本文重绘：DiT-XL/2 在 256×256 图像上的完整张量形状——从 4×32×32 latent 到 256 个 1152 维 token，再还原为 8×32×32 输出](assets/dit-tensor-shapes.svg)
...
![本文重绘：adaLN-Zero 的微观数据流——条件向量产生两组 scale、shift 与 gate，分别控制 Attention 和 MLP 残差分支](assets/dit-adaln-zero-block.svg)
...
![本文重绘：DiT 训练与推理的差别——训练随机抽一个时间步并一次前向求损失，推理从纯噪声出发多次调用共享参数的 DiT](assets/dit-train-vs-infer.svg)
...
![本文重绘：DiT 原论文的证据链——adaLN-Zero 的条件机制优势、Gflops 与 FID 的强负相关，以及增大采样步数不能补偿小模型能力](assets/dit-evidence.svg)

### 证据一：条件注入不是小细节

读这张原始曲线时，先比较四条线是否使用同一 XL/2 主干，再看纵轴 FID-50K（越低越好）和训练步数；蓝线若始终最低，才能把优势归因到条件接线，而不是模型规模。

![论文原图 Figure 4：四种条件注入方式在训练过程中的 FID-50K](assets/dit-paper-fig04-conditioning.png)

*论文原图 Figure 4，来源：[DiT 论文](https://arxiv.org/abs/2212.09748)。adaLN-Zero 从早期训练到 400K step 都优于三个对照，直接支持“条件注入是关键设计变量”。它不能单独证明 Cross-Attention 对长文本无效，因为这里的条件只有时间与类别，任务和条件带宽都更窄。*

在相同的 DiT-XL/2 规模下，adaLN-Zero 在全部训练阶段优于 in-context、Cross-Attention 和普通 adaLN。到 400K step，论文正文称它的 FID **接近 in-context 的一半**；从 Figure 4 读图约为 19 对 35。Cross-Attention 还带来约 15% 的额外 Gflops，而 adaLN-Zero 增量可以忽略。
...
看下面两幅散点时，不要只追着模型名走：横轴是一次前向的 Gflops，纵轴仍是越低越好的 FID；颜色和点型把模型宽深与 patch 大小造成的计算变化放在同一坐标中。

![论文原图 Figure 7：DiT 前向计算量与 FID-50K 的关系](assets/dit-paper-fig07-gflops-fid.png)

*论文原图 Figure 7，来源：[DiT 论文](https://arxiv.org/abs/2212.09748)。样本点整体向右下排列，支持论文在这 12 个配置上报告的强负相关；同尺寸模型缩小 patch 也会增加计算并改善 FID。它仍是同一家族、同一训练方案内的相关性证据，不能推出跨任务的“FLOPs 越多必然越好”，也没有完全分离更多计算与更细空间离散化的贡献。*

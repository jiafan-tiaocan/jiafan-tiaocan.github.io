...
> [!info] 与底层论文证据互读
> [SVD 论文解读](/aigc/论文解读：stable-video-diffusion:-scaling-latent-video-diffusion-models-to-large-datasets)说明底座如何通过图像预训练、筛选视频预训练和高质量微调获得运动先验。符号上，SVD 的视频噪声 $z_t$、重复到每一帧的参考图 latent $z_{\mathrm{cond}}$ 与 CLIP 图像条件，对应下文 U-Net 的噪声输入、参考图拼接分支与身份语义分支；FPS、motion bucket 和 cond augmentation 对应附加时间条件 $m$。SVD 证明的是 I2V 底座和数据课程，不证明给定骨架能被精确执行。
>

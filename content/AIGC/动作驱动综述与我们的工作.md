...
DWPose 在这里对应的是上游 2D 全身姿态估计器：它先检测人体，再为每个人输出身体、脚、脸和双手共 133 个关键点及分数。[DWPose 原论文](https://openaccess.thecvf.com/content/ICCV2023W/CV4Metaverse/html/Yang_Effective_Whole-Body_Pose_Estimation_with_Two-Stages_Distillation_ICCVW_2023_paper.html)表明，原论文直接验证的是 UBody 增量数据与两阶段蒸馏对 COCO-WholeBody AP 的提升，并没有证明这些分数已经严格校准，也没有评测视频时序稳定性。下面把分数渲染为姿态亮度、筛选高置信手部并写入区域 Loss，属于 MimicMotion 与项目侧对 DWPose 输出的进一步使用；跨 Clip 融合、脸部加权和业务数据微调更超出了 DWPose 论文证据。

> [!info] 与底层论文证据互读
> [SVD 论文解读](https://jiafan-tiaocan.github.io/aigc/论文解读：stable-video-diffusion:-scaling-latent-video-diffusion-models-to-large-datasets)说明底座如何通过图像预训练、筛选视频预训练和高质量微调获得运动先验。符号上，SVD 的视频噪声 $z_t$、重复到每一帧的参考图 latent $z_{\mathrm{cond}}$ 与 CLIP 图像条件，对应下文 U-Net 的噪声输入、参考图拼接分支与身份语义分支；FPS、motion bucket 和 cond augmentation 对应附加时间条件 $m$。SVD 证明的是 I2V 底座和数据课程，不证明给定骨架能被精确执行。
>
> [MimicMotion 原论文](https://arxiv.org/abs/2406.19680)及其固定版本官方代码共同给出了模型结构、训练设定和因果消融。它在 SVD 上新增 PoseNet，论文的参考图 $I_{\mathrm{ref}}$、姿态 $P$ 与窗口 latent $z_i^j$ 分别对应本项目的人物图、DWPose 序列与分段 Clip 状态。论文验证的是 4,436 段舞蹈视频训练、TikTok 335–340 测试下的置信度姿态、高置信手区 10× loss 和渐进融合；项目的 7,000+ 绿幕口播数据、脸部 2× loss、末帧继承、FreeNoise 与口型改写都是论文之外的系统实践，不能写成论文已经证明的能力。

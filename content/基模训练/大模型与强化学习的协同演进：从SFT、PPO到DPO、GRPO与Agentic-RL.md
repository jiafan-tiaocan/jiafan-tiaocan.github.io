---
title: 大模型与强化学习的协同演进
description: 从按钮游戏和策略梯度的逐行推导开始，讲清 PPO、DPO、GRPO 的演进，并区分预训练、SFT、RL 与运行时工具分别让大模型获得什么，以及万卡集群如何做阶段规划。
tags:
  - 大模型
  - 强化学习
  - LLM-Post-Training
...
第五章的数学题只有一个 prompt、若干完整回答和终局验证。但“模型都在逐 Token 或逐动作生成”不代表它们面对的是同一种强化学习问题。真正决定 critic 是否值得额外成本的，不是模型叫 VLM、VLA 还是 Agent，而是：**一次动作会不会改变外部环境，并让下一步面对一个新的状态。**

## 6.1 一次性交卷，还是每一步都改变下一道题

先比较知识库里的两个真实应用。

在[[图生模版：从多模型工作流到端到端视觉语言模型|图生模版]]中，输入是一张固定图片 $I$，模型输出一份包含元素角色、坐标和层级的完整结构 $y$，verifier 再计算格式、集合与 IoU 奖励：

$$
I\longrightarrow y\longrightarrow R(I,y)
$$

模型内部虽然仍然逐 Token 生成 JSON，但写出一个错误坐标不会移动原图里的元素，环境也不会返回一张被修改后的新图片。完整 JSON 因而可以看成一次宏观动作：模型看一次题目、提交一份答卷、接受一次判分。这类“给定上下文后做一次完整选择”的问题通常称为 contextual bandit。

对同一张图片生成 $G$ 份结构，所有候选面对完全相同的输入和标准标注。固定 verifier 得到奖励 $R_1,\ldots,R_G$ 后，组内平均就能近似“模型在这张图上的通常水平”：

$$
b(I)=\frac{1}{G}\sum_{i=1}^{G}R_i,
\qquad
\hat A_i=R_i-b(I)
$$

这里不必额外训练一个 critic 去猜每个 JSON 前缀最终能得几分，主要有三个原因：

1. **条件几乎不变**：所有候选始终对应同一张图片，没有策略动作造成的新外部状态；
2. **完整采样相对便宜**：多生成几份 JSON 主要增加推理计算，不需要复位物理世界；
3. **结果可以复算**：Schema、元素集合和 IoU verifier 对同一输出通常会给出相同结果。

这不表示图生模版已经拥有完美奖励。平均 IoU 不知道角色是否错配，也不能完整表达遮挡、分组、层级和最终可编辑性，所以奖励必须先经过解析、集合与角色门禁。但这些缺口首先要求改进 verifier 和数据契约，并不会自动证明需要一个 value model。

VLA 的结构不同。以[[VLA技术演进与最新模型版图|VLA]]中的“把红色易拉罐放进绿色篮子”为例，机器人每执行一次动作，物体、机械臂、相机画面和本体状态都可能变化：

$$
s_t\xrightarrow{a_t}s_{t+1}\xrightarrow{a_{t+1}}s_{t+2}
$$

$s_t$ 包含当前视觉、本体状态与动作历史，$a_t$ 是当前动作。向左靠近与向右靠近会进入不同的 $s_{t+1}$；抓取时把罐子碰偏，下一帧看到的就不再是原来的题目。策略不是在静态输入上反复提交答案，而是在亲手改写下一步的输入。

同一个终局失败分数也可能来自完全不同的原因：

- 第一段靠近方向就错了；
- 已经正确抓住，只在最后放置时松手；
- 动作本身合理，但相机丢帧或环境被外力干扰；
- 轨迹被超时截断，还没有走到可以判定成败的位置。

若这些轨迹最后都得到 $R=0$，只比较完整轨迹仍能判断“哪条整体更好”，却很难判断一条长轨迹中哪些早期动作应该保留、哪些动作真正导致失败。这就是长程信用分配问题。

| 反馈结构 | 图生模版 | VLA |
|---|---|---|
| 动作会不会改变外部环境 | 不会，原图保持固定 | 会，机器人和物体进入新状态 |
| 过程中是否收到新观测 | 通常没有，生成完再验证 | 每执行一段动作就重新观察 |
| 完整 rollout 的成本 | 多生成一份结构化输出 | 重新执行、复位并承担时间与硬件风险 |
| 奖励噪声 | 规则通常可重复计算 | 接触、传感器、复位和环境都可能引入噪声 |
| 同组样本是否严格可比 | 通常共享同一图片和真值 | 初始状态与环境扰动未必完全相同 |
| critic 的边际价值 | 通常低于增加采样和改进 verifier | 长轨迹、稀疏奖励时可能更高 |

## 6.2 Critic 多提供的是“当前状态还有多大希望”

GRPO 的组内 baseline 主要回答：

> 从同一个初始题目出发，这条完整轨迹是否比其他完整轨迹更好？

若只在初始状态 $s_0$ 采样一组轨迹，它近似的是：

$$
b(s_0)\approx\mathbb E[R\mid s_0]
$$

但 VLA 执行几步以后，各条轨迹已经进入不同的 $s_t$。此时更需要一个会随状态变化的 baseline：

$$
V(s_t)=\mathbb E[G_t\mid s_t]
$$

它回答“机器人已经走到当前状态，后面继续执行通常还能得到多少回报”。看一个只有数字的例子。假设机器人尚未获得任何即时奖励，当前状态的预测价值是：

$$
V(s_t)=0.5
$$

动作 A 让夹爪与罐子更好地对齐，下一状态价值变成：

$$
V(s_{t+1}^{A})=0.8
$$

动作 B 把罐子推向遮挡区域，下一状态价值变成：

$$
V(s_{t+1}^{B})=0.2
$$

此时任务还没完成，所以 $r_t=0$。取 $\gamma=1$，两个动作的 TD 误差分别是：
...
终局成功尚未发生，critic 已经提供了不同的中间信号：动作 A 把系统带入更有希望的状态，动作 B 把系统带入更差的状态。它不是凭空知道正确动作，而是从历史轨迹中学习“处于哪些状态时，后面通常更容易成功”。

因此，组内 baseline 与 critic 回答的是两个不同粒度的问题：

$$
\underbrace{R_i-\operatorname{mean}(R)}_{\text{完整轨迹相对同组好不好}}
\qquad\text{与}\qquad
\underbrace{r_t+\gamma V(s_{t+1})-V(s_t)}_{\text{这一步是否改善了后续前景}}
$$

critic 还可以从未走到终点的截断轨迹 bootstrap，减少每次都必须执行完整物理任务的压力。代价是它自己也可能估错：错误的 $V(s_t)$ 会把偏差写进 advantage，所以“轨迹长”并不自动等于“必须训练 critic”。

更准确的判断是：

- 仿真便宜、能从同一状态并行采样、只关心终局成功时，VLA 仍可以使用 RLOO/GRPO、rejection sampling 或离线优势学习；
- 真实 rollout 昂贵、奖励延迟、轨迹经常截断，而且中间状态质量可以学习时，actor-critic / value-based 方法更可能值得成本；
- 如果图生模版以后变成多轮编辑 Agent——调用分割、查看 mask、修改图层、渲染后继续调整——它也会进入动作改变状态的闭环，此时过程奖励或 critic 同样可能有价值。

所以算法选择不应写成“VLA 需要 critic，图生模版不需要”，而应检查四个条件：
...
Agent 的搜索、工具调用和代码执行也是同一结构。动作会产生新的外部 observation，完整轨迹可以写成：
...
# 七、预训练、SFT 与 RL 分别让模型学到了什么

“不要幻觉是 SFT，更有逻辑、不自相矛盾是 RL”抓到了一部分现象，但不能作为训练分工。它把四件不同的事压成了两件：

1. **预训练建立能力**：模型是否见过相关事实、语言结构、代码和推理模式；
2. **SFT 建立行为协议**：面对指令时，模型默认应以什么格式、语气和步骤作答；
3. **RL 重排行为概率**：模型自己生成多种答案时，哪些策略因为结果更好而应更常出现；
4. **运行时系统提供证据**：搜索、RAG、数据库、工具、记忆和独立 verifier 给出模型参数中没有的最新事实与外部约束。

前三项都在改变参数，但改变参数不等于凭空获得真实世界证据。后面讨论“幻觉”时必须保留这个边界。

## 7.1 预训练主要建立知识、语言与潜在能力

预训练从海量 Token 中学习“给定前文，下一个 Token 是什么”：
...
因为语料里同时存在事实叙述、论证、代码、数学证明和对话，模型会逐渐获得：

- 词义、语法、文体和多语言表示；
- 世界知识、领域术语以及概念之间的统计联系；
- 代码模式、计算步骤和可以被提示唤起的潜在算法；
- 从上下文归纳任务、模仿格式和续写长结构的能力。

但它的训练目标只是**预测语料中更可能出现什么**，不是判断命题在现实中是否为真。语料会过时、互相冲突，也会同时包含事实和错误；在证据不足时，流畅续写仍可能比“我不知道”更符合似然。因此，幻觉不是只靠后训练就能彻底清除的表面坏习惯，它也来自知识覆盖、数据质量、目标函数和运行时证据缺失。

[LIMA](https://arxiv.org/abs/2305.11206) 用仅 1,000 个精心选择的示范得到很强的指令跟随效果，并据此提出一种有影响力的解释：大部分知识与能力在预训练中获得，后训练主要学习交互格式。这是支持分层理解的实验证据，不是“少量 SFT 对所有模型都够用”的定律。

## 7.2 SFT 主要教“理想回答长什么样、应该怎样表现”
...
- 把预训练中已经存在但不稳定的知识和能力组织成可用回答；
- 学习 JSON、工具 schema、引用格式、语言风格和拒答边界；
- 通过标准解题过程、工具轨迹和错误修正示范，建立可读的初始策略；
- 学会“证据不足时说明不确定”“先核验再回答”等诚实行为协议。

所以，“SFT 可以减少幻觉”在**行为层**是成立的：它可以示范何时引用、何时拒答、怎样区分事实与猜测。但 SFT 并不保证回答中的事实为真。过时知识没有因此自动更新；如果训练集给了错误答案，SFT 还会更坚定地模仿错误。

它另一个盲点是 teacher forcing：训练时每一步都站在专家给出的正确前缀上。模型自己生成错误前缀后如何发现矛盾、回退并恢复，如果示范没有覆盖，它就没有直接经历过。

## 7.3 RL 主要教“哪些自身行为更值得重复”
...
- 在多种可行答案、推理路线和工具序列之间重新分配概率；
- 利用失败、比较和环境结果，而不只学习专家正例；
- 优化不可微指标，如测试通过率、任务成功率和人类偏好；
- 在模型自己造成的状态中学习重试、验证、预算分配和错误恢复。

这解释了 RL 为什么经常改善可验证推理。[DeepSeek-R1](https://www.nature.com/articles/s41586-025-09422-z) 报告，R1-Zero 从 base model 直接进行大规模 RL 后出现了自验证、反思和动态调整策略，但可读性差且存在语言混杂；完整 R1 又加入 cold-start SFT、拒绝采样后的 SFT 和第二阶段 RL。这个对照更接近真实分工：**预训练先提供潜在推理能力，SFT 给出可读起点，RL 再让可得高分的策略更常被采样。**

“奖励只看最终答案”也未必足以训练出可靠逻辑。错误步骤可能碰巧得到正确答案，最终正确也不能指出错误从哪里开始。[Let's Verify Step by Step](https://arxiv.org/abs/2305.20050) 对比了 outcome supervision 与 process supervision，说明步骤级反馈可以更直接地定位推理过程。因此，是否能减少逻辑跳步或内部矛盾，关键不只在“用了 RL”，还在 reward 或 verifier **能不能看见这些错误**。

RL 的边界同样明确：reward 只说“在当前评分器下得分高不高”，不会自动提供正确知识。base model 几乎采样不到成功轨迹时，policy gradient 无米下锅；verifier 有漏洞时，RL 会把漏洞利用得越来越熟练。如果 reward 只奖励语气自信，RL 甚至可能让幻觉更严重。

![预训练建立知识与潜在能力，SFT建立回答协议，强化学习依据结果重排模型自身轨迹，运行时工具提供最新证据](assets/sft-vs-rl-learning.svg)

*图 8　四层分工。预训练决定模型大致“会不会”，SFT 决定它默认“怎样做”，RL 在可评价的自身轨迹中决定“哪些做法更值得重复”；搜索、RAG、工具与独立 verifier 则补充参数之外的事实和约束。它们相互补充，不是互斥选项。本文归纳。*

## 7.4 “更有逻辑、少幻觉、不矛盾”究竟该优化哪一层

| 目标症状 | 预训练解决什么 | SFT 解决什么 | RL 解决什么 | 运行时系统解决什么 |
|---|---|---|---|---|
| **少事实幻觉** | 扩大可靠知识覆盖，修正错误与冲突语料 | 示范引用、核验、承认未知和拒答 | 在事实可核验时，降低错误断言的概率 | 搜索/RAG/数据库提供当前证据，verifier 检查主张 |
| **更有逻辑** | 提供语言、数学、代码与推理的潜在能力 | 示范清晰分解、正确中间步骤和表达方式 | 在可靠结果或过程反馈下，强化有效路线、自检与回退 | 计算器、代码执行器、定理证明器验证中间结果 |
| **少自相矛盾** | 学习长程依赖和语义一致性 | 示范先列约束、前后一致检查和修订 | reward 能检测矛盾时，惩罚不一致轨迹 | 结构化状态、记忆与一致性 checker 保存和核对前文承诺 |
| **知道何时不知道** | 见过知识边界和不同证据强度的表达 | 示范校准措辞与合适拒答 | 用可验证反馈校准答错、拒答和求助的代价 | 检索置信度、来源质量和人工升级提供决策依据 |

因此，最准确的回答不是“幻觉归 SFT，逻辑归 RL”，而是：
...
- “不要幻觉主要靠 SFT”只在**教模型如何诚实表现**这一层部分成立；事实是否正确还取决于预训练知识和当前证据。
- “逻辑与一致性主要靠 RL”只在**已有基本能力，而且 reward 真能区分好坏过程**时部分成立；基础逻辑能力仍来自预训练，清晰推导格式仍可由 SFT 高效传授。
- 任何一层都不应许诺“零幻觉”。合理目标是按失败类型分别降低发生率，并保留可核验来源、拒答与人工升级路径。

## 7.5 症状决定优化方式

| 真实症状 | 首先应该优化什么 | 原因 |
|---|---|---|
| 大量领域事实和术语根本没见过 | 清洗领域语料后持续预训练；规模很小且稳定时可辅以 SFT | 先补表示与知识覆盖，稀疏 reward 不适合承担大规模知识写入 |
| 信息会随日期、库存、政策或用户状态变化 | 搜索/RAG/数据库 + 引用 SFT + 事实性评测 | 参数训练无法保证拿到提问时刻的外部状态 |
| 不会按格式输出、不会合法调用工具 | SFT | 示范给逐 Token 稠密信号，样本效率最高 |
| 会做题但路线不稳定、不会自检和回退 | 可验证 SFT 打底，再做 process/outcome RL | 需要在自身轨迹中比较策略，前提是反馈能识别好坏 |
| 长答案前后矛盾 | 一致性示范 + 结构化状态/verifier；有可靠反馈后再做 RL | 先让矛盾可检测、可归因，才能安全地优化 |
...
这不是必须执行到底的流水线。每次升级都应先问：前一个方法缺的是知识、当前证据、行为示范、探索、负样本、信用分配，还是仅仅缺更好的评测？
...
## 8.2 如果有一万张卡，三个阶段各需要多少

先给结论：**“万卡集群”不是三个阶段都应该塞满一万张卡。** 预训练的计算密度高、数据量巨大，通常最适合大规模并行；SFT 数据少得多，用满集群会遇到数据重复和扩展效率问题；RL 的瓶颈又常从反向计算转向自回归 rollout、模型副本、环境和 verifier。合理分配取决于每个阶段的瓶颈，而不是让利用率数字始终接近 100%。

卡数和时间至少由以下变量共同决定：GPU 型号、dense 或 MoE、总参数与激活参数、训练 Token、上下文和回答长度、精度、并行策略、MFU、集群网络、故障率，以及 RL 中每题采样数与 reward 成本。脱离这些条件报一个天数，会制造虚假的确定性。

### 8.2.1 先用公开账单建立数量级

把公开 GPU-hour 机械地摊到 $N_{\mathrm{GPU}}$ 张同型卡上，只能得到理想墙钟时间：

$$
T_{\mathrm{days}}
=\frac{C_{\mathrm{GPU\text{-}hour}}}
{24N_{\mathrm{GPU}}}
$$

| 公开项目 | 公开训练量与 GPU-hour | 机械换算到 10,000 张同型卡 | 必须保留的边界 |
|---|---:|---:|---|
| Llama 3.1 70B | 约 15T 预训练 Token；累计约 7.0M H100-80GB GPU-hour | 约 29.2 天 | 模型卡表格给的是累计训练量，未逐项拆分预训练与后训练；真实大规模扩展也不可能完全线性 |
| Llama 3.1 405B | 累计约 30.84M H100-80GB GPU-hour | 约 128.5 天 | 参数量、并行通信和故障恢复使它不能按 70B 比例简单缩放 |
| DeepSeek-V3 | 预训练 2.664M、长上下文扩展 119K、后训练 5K H800 GPU-hour | 预训练约 11.1 天 | 它是 671B 总参数、37B 激活参数的 MoE；H800 不能与 H100 等价比较 |

[Llama 3.1 模型卡](https://huggingface.co/meta-llama/Llama-3.1-405B-Instruct#hardware-and-software) 给出了约 15T Token 和累计 H100 GPU-hour。[DeepSeek-V3 技术报告](https://arxiv.org/abs/2412.19437) 报告实际使用 2,048 张 H800，预训练少于两个月；$2.664\text{M}/2{,}048\approx1{,}301$ 小时，即约 54.2 天，与这个描述相符。它也明确说明账单只计正式训练，不包含此前研究和消融。这里的换算只用于校验数量级，**不能拿来宣称某种架构一定更省钱**。

### 8.2.2 一个可讨论的工程案例

下面固定一个场景，避免不断偷换条件：

> 从头训练一个 dense 70B 模型，约 15T 预训练 Token，硬件为 H100 80GB；集群共有 10,000 张卡，训练栈已经稳定。以下时间指“单次成功主训练”的规划量级，不包括数据采购、清洗、失败实验、超参搜索和产品验收。

#### 阶段一：预训练

以 Llama 3.1 70B 的 7.0M H100 GPU-hour 作为粗锚点：

$$
\frac{7.0\times10^6}{8{,}192}
\approx854.5\ \text{hours}
\approx35.6\ \text{days}
$$

因此更现实的规划是：

- **主训练使用 8,192—10,000 张卡，单次成功 run 约 4—6 周**；
- 若拓扑以 8,192 张卡最稳定，就不要为了“用满一万卡”强行扩大同步域；其余资源可承担数据去重、在线评测、checkpoint 校验、故障替补和小规模消融；
- 项目日历一定长于 4—6 周，因为数据管线验证、短程 scaling run、故障重启和模型评测不在上述 GPU-hour 的机械换算里。

这也是三个阶段中最值得使用全量集群的阶段：15T Token 足以持续喂饱并行训练，前向、反向和优化器计算都能形成大批量流水线。

#### 阶段二：SFT

SFT 仍是 teacher-forced 前向与反向，计算形态接近预训练，但有效 Token 通常少几个数量级。若只为建立数量级，在**同一模型、硬件、精度、序列形状和相近 MFU**这一组很强的假设下，可以按 Token 粗缩放：

$$
C_{\mathrm{SFT}}
\approx C_{\mathrm{PT}}
\frac{D_{\mathrm{SFT}}}{D_{\mathrm{PT}}}
$$

例如把 10B—50B 个有效 SFT Token 代入 15T Token、7.0M H100 GPU-hour 的锚点，机械结果约为 4.7K—23.3K H100 GPU-hour。放到 1,024 张卡上约 4.6—22.8 小时，但这不是交付时间：数据混合、长短样本 padding、验证、checkpoint 和多次配方迭代都会占更多时间。

更合理的规划是：

- **每次 SFT run 使用 512—2,048 张卡，约 0.5—2 天**；
- **整个 SFT 阶段预留 1—2 周**，重点用于数据配比、能力回归、拒答边界和多轮迭代，而不是持续跑同一个 job；
- 剩余卡应并行做数据生成与过滤、独立评测、蒸馏教师推理，或服务其他训练任务。仅为了提高表面利用率而重复小数据，可能加剧过拟合和能力遗忘。

这里也能看出 DPO 的计算位置：虽然它在本文的“偏好优化”主线里出现，但工程形态更接近带 chosen/rejected 双回答的离线微调，通常不需要在线 rollout，也不应该按长程 RL 的资源方式估算。

#### 阶段三：RL

RL 不能再仅按训练 Token 比例估算。一次在线 RL 先要逐 Token 生成，再计算 reward、reference log-prob 和可能存在的 value，最后才反向更新 policy。rollout 数据量至少是：

$$
D_{\mathrm{rollout}}
=N_{\mathrm{step}}
\times N_{\mathrm{prompt/step}}
\times G
\times \overline{L}_{\mathrm{response}}
$$

其中 $G$ 是同题采样数，$\overline{L}_{\mathrm{response}}$ 是平均回答长度。自回归生成难以像 teacher forcing 那样把所有 Token 一次并行算完；回答越长，KV cache 和尾部慢样本越容易成为瓶颈。PPO 还需要 value model，GRPO 虽省去 critic，却可能通过更大的 $G$ 增加采样量。

[DeepSeek-R1](https://www.nature.com/articles/s41586-025-09422-z) 公开了 R1-Zero 的部分训练配置：共训练 10,400 个 step，每题采样 16 个输出，每个训练 step 含 32 道题、即 512 条样本；每轮 rollout 生成 8,192 条输出，再切成 16 个 minibatch。最大回答长度先为 32,768，后扩展到 65,536 Token。但论文没有同时披露平均实际长度、端到端 rollout 吞吐和各模型副本的卡数，所以这些数字**不能诚实地换算成精确墙钟时间**。

对于上面的 70B 案例，可以把工程预算写成区间，而不是伪装成论文事实：

- **短回答偏好或可验证 RL：2,048—4,096 张卡，单次 run 约 3—10 天**；
- **长 CoT 的 GRPO/PPO：4,096—8,192 张卡，单次 run 约 1—4 周**；
- 在一个占用全部 10,000 张卡的长 CoT 阶段，可先按 **50% rollout、25% policy learner、10% reference/reward/verifier、10% 独立评测与数据、5% 故障冗余** 建立容量模型，再根据 rollout 队列和 learner 空闲率动态调整。

这个比例不是算法常数。规则 verifier 可能主要消耗 CPU，从而把更多 GPU 还给 rollout；大 reward model 与 PPO critic 会增加模型副本；多轮 Agent 或 VLA 还会被环境速度、仿真器和真实机器人吞吐限制，额外 GPU 未必能缩短日历时间。

### 8.2.3 把规划表压缩成一句话

| 阶段 | 70B / H100 案例的合理峰值卡数 | 单次成功主 run | 为什么不是同一种分配 |
|---|---:|---:|---|
| 预训练 | 8,192—10,000 | 约 4—6 周 | Token 海量、计算密集，最能利用大同步集群 |
| SFT | 512—2,048 | 约 0.5—2 天；阶段迭代 1—2 周 | 数据较少，瓶颈转向配方与评测，用满集群收益低 |
| 短回答在线 RL | 2,048—4,096 | 约 3—10 天 | rollout、reward 与训练需要流水化 |
| 长 CoT / Agentic RL | 4,096—8,192；必要时全池分角色 | 约 1—4 周 | 长序列生成、模型副本、环境和 verifier 主导成本 |

这里最容易混淆的是“这次正式 run 用了多久”和“组织为得到这次 run 花了多久”。公开论文偶尔披露前者，却很少完整披露失败实验、数据生产、reward 调试和评测成本。做集群预算时，两笔账必须分开。
...
最终选择仍然很朴素：**需要扩大知识覆盖与基础能力，回到预训练数据；需要规定理想回答和接口行为，用 SFT；需要模型在自己的行为里根据结果做选择、探索和恢复，用 RL；需要回答此刻的外部事实，就接入搜索、RAG、工具与独立核验。** 而在优化任何 reward 之前，先证明这个 reward 看得见真正关心的错误，也值得被优化。

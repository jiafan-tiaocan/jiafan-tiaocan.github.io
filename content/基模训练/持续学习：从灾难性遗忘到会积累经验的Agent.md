...
本文内容核验截至 **2026-07-27**。如果你只想建立直觉，读第 0—3 节和结论；想掌握算法，继续读第 4—7 节；关心大模型与 Agent，重点读第 8—10 节；准备落地，直接进入第 11 节的实验与工程清单。

> [!warning] 证据边界
> - 梁文锋的发言来自媒体整理的闭门会实录，不是官方论文或产品承诺；本文把它当作研究问题的契机，不把它当成技术已经实现的证据。
> - 经典持续学习实验大多在人工切分的任务序列上进行；在这些基准上有效，不等于在开放世界、多人生产系统中同样有效。
> - Voyager 等 Agent 通过外部技能库积累能力，说明**系统级**持续学习可行；它没有让底层 GPT-4 权重在线自我更新。
> - 梁文锋关于持续学习、自我迭代和 AGI 的关系是一条研究假设，不是已经被领域证明的必然阶梯。
...
| 维度 | 要问的问题 | 可观测量 |
|---|---|---|
| 可塑性 | 新知识学得多快、样本效率是否下降 | 学习曲线面积、达到阈值所需样本、与从头训练差值 |

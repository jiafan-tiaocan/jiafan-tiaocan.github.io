...
![KV-cache 从单序列增量复用到 Agent 跨会话基础设施的演进路线](AR与KV-cache.assets/01-kv-cache-evolution.png)
...
![PagedAttention 中逻辑 KV 块到非连续物理块的映射](AR与KV-cache.assets/02-pagedattention-figure5.png)
...
![RadixAttention 用基数树维护共享前缀及其 KV-cache](AR与KV-cache.assets/03-radixattention-figure3.png)
...
![Mooncake 以解耦 KVCache 为中心的推理架构](AR与KV-cache.assets/04-mooncake-figure1.png)
...
![Agent 跨会话 KV-cache 的语义平面、控制面、推理数据面与分层存储](AR与KV-cache.assets/05-agent-cross-session-architecture.png)

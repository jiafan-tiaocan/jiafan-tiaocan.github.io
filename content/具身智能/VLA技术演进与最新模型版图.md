...
description: 从一个机械臂的具体输入输出出发，沿语义迁移、动作表示、跨本体数据、层级推理与在线经验五条主线，梳理 VLA 的技术演进及最新模型版图。
tags:
  - 具身智能
  - VLA
  - Robot-Learning
...
这也解释了为什么不存在一个对所有机器人、任务和部署条件都“最新最好”的 VLA。截至 **2026-07-29**，至少需要区分四类前沿：
...
[![VLA 技术演进图](assets/vla-evolution-map.svg)](assets/vla-evolution-map.svg)
...
> [!tip] 阅读路线
> - **第一次接触 VLA**：先读第零章和第一章，弄清视觉、语言、本体状态与动作输出分别是什么。
> - **想理解技术为什么演进**：读第二、三章，重点看动作 Token 为什么又让位于连续动作专家。
> - **关心最新模型与选型**：直接读第五、六章。
> - **关心 Thinking、长期记忆与复杂任务**：读第七、八章。

> [!note] 证据边界
> 本文中的模型结构、数据规模和实验结果来自论文、技术报告或官方仓库；跨模型的因果路线属于本文归纳。厂商自评只说明其报告条件下的结果，不构成跨机器人、跨任务的统一排名。最新版本状态核验于 2026-07-29，Frontmatter 的 `date` 仍表示文章创建时间。

# 零、先用一个机械臂案例看懂 VLM 与 VLA

## 0.1 两者不是“多了一个输出字母”这么简单

先想象桌面上有一个红色易拉罐和一个绿色篮子。

如果给 **VLM** 一张相机图像，并问“红色易拉罐在哪里？”，它通常输出文本 Token，例如：

> 红色易拉罐在绿色篮子的左侧。

这里的输入是“图像 + 文本问题”，输出仍是文本、框、点或其他语义结果。模型可以描述动作，却不会因此让机械臂真的移动。

如果给 **VLA** 同一张图像，并下达“把红色易拉罐放进绿色篮子”的指令，它还要读取机器人的当前关节角、夹爪状态或末端位姿，然后输出机器人在未来若干控制步应该执行的动作。严格说，VLA 常见输入不只是 Vision 和 Language 两项：

| 输入 | 这个案例里是什么 | 为什么需要 |
|---|---|---|
| 视觉观测 $I_{t-k:t}$ | 头部/腕部相机最近几帧 | 确定罐子、篮子、机械臂和遮挡关系 |
| 语言指令 $\ell$ | “把红色易拉罐放进绿色篮子” | 指定对象、目标位置与任务约束 |
| 本体状态 $q_t$ | 关节角、末端位姿、夹爪开合 | 判断机器人现在在哪里、还能怎样移动 |
| 历史与本体标识 $e$ | 过去动作、机器人型号、控制模式 | 消除速度、自由度和动作空间差异 |

VLA 的原始输出可能是离散动作 Token，也可能是连续动作块；解码后，它们才变成末端位姿增量、关节目标、夹爪命令或底盘速度。底层控制器再把这些目标转换成更高频的关节伺服或电机指令。

因此，“Action 输出”至少有三个层次：

| 层次 | 可能看到的形式 | 谁负责产生或转换 |
|---|---|---|
| 模型原始输出 | `132 114 128 5 25 156`，或 $H\times d$ 连续张量 | VLA 主干或动作专家 |
| 机器人动作目标 | 末端平移/旋转、关节位置、夹爪、底盘速度 | 动作解码器与机器人适配层 |
| 电机级控制 | 关节位置环、速度环或力矩命令 | 低层控制器，不一定由 VLA 直接输出 |

## 0.2 一个具体的 7 维动作输出

假设这台单臂机器人采用常见的“末端增量 + 夹爪”接口，一个控制步可以写成：
...
前三维表示末端执行器沿 $x,y,z$ 方向的位移，接着三维表示绕三个轴的旋转增量，$g$ 表示夹爪目标。不同机器人可能改用绝对位姿、关节角、关节速度、底盘速度或全身控制目标，因此 **7 维只是具体例子，不是 VLA 的统一标准**。

下面是一段用于解释数据格式的四步动作块。这里约定平移单位为米、旋转单位为弧度，$g=0$ 表示保持张开、$g=1$ 表示闭合：

| 步 | 机械含义 | $[\Delta x,\Delta y,\Delta z,\Delta\phi,\Delta\theta,\Delta\psi,g]$ |
|---:|---|---|
| 0 | 靠近易拉罐 | `[0.012, -0.004, 0.006, 0.00, 0.03, -0.01, 0]` |
| 1 | 向下对准抓取位 | `[0.004, 0.000, -0.018, 0.00, 0.00, 0.00, 0]` |
| 2 | 闭合夹爪 | `[0.000, 0.000, 0.000, 0.00, 0.00, 0.00, 1]` |
| 3 | 抬起易拉罐 | `[0.000, 0.000, 0.025, 0.00, 0.00, 0.00, 1]` |

这些数字只是**接口级示意**，不是某次论文实验的真实日志，也不能直接下发给真实机器人。它们要经过坐标系转换、限幅、碰撞检查和底层控制器。

下面的标准 Python 代码可以直接运行，用来检查“动作块是一个 $H\times7$ 数组”这件事；它只打印动作，不连接任何机器人 SDK：
...
action_chunk = [
    EndEffectorAction(0.012, -0.004, 0.006, 0.00, 0.03, -0.01, 0.0),
    EndEffectorAction(0.004, 0.000, -0.018, 0.00, 0.00, 0.00, 0.0),
    EndEffectorAction(0.000, 0.000, 0.000, 0.00, 0.00, 0.00, 1.0),
    EndEffectorAction(0.000, 0.000, 0.025, 0.00, 0.00, 0.00, 1.0),
]

assert all(0.0 <= action.gripper <= 1.0 for action in action_chunk)

# 滚动闭环通常只执行动作块前几步，然后读取新图像重新预测。
for step, action in enumerate(action_chunk[:2]):
    print(step, action)
```

真实闭环不会盲目执行完整动作块。模型可能预测未来 8、16 或更多步，但机器人只执行前几步：如果易拉罐被碰偏，新相机帧会进入下一次推理，VLA 随即修正接近方向。

## 0.3 RT-2 的输出为什么看起来像“文本”

读下面的原论文图时，重点看中间下方的紫色路径：RT-2 首先输出一串整数 Token，再把这些 Token 反量化为平移 $\Delta T$ 和旋转 $\Delta R$。所以它的神经网络接口像语言模型，最终物理含义却是机器人动作。

[![RT-2 Figure 1：图像和语言输入经 VLM 生成离散动作 Token，再反量化为平移与旋转命令并闭环控制机械臂](assets/vla-io-evidence/01-rt2-figure1-action-tokens.png)](assets/vla-io-evidence/01-rt2-figure1-action-tokens.png)

*图 2　RT-2 把网页视觉问答和机器人动作数据共同微调；中间的动作 Token 经 de-tokenize 还原为机器人动作，右侧展示闭环执行。裁剪自 [RT-2 原论文 Figure 1](https://arxiv.org/abs/2307.15818)，版权归原作者与 Google DeepMind。*

图左上仍是传统 VLM 任务：输入图像和问题，输出文本答案。左下把机器人图像和任务指令配上一串动作 Token。中间同一个视觉语言骨干既能生成自然语言，也能生成动作 Token；右侧机器人执行后产生新图像，再进入下一轮推理。

这也揭示了 VLM 到 VLA 的严格变化：

$$
\underbrace{\{\text{image},\text{text}\}\rightarrow\text{text}}_{\text{VLM}}
\qquad\Longrightarrow\qquad
\underbrace{\{\text{image history},\text{instruction},\text{robot state}\}
\rightarrow\text{action chunk}}_{\text{VLA}}
$$

后续模型保留了 VLM 的语义骨干，却不一定继续把动作伪装成文字。$\pi_0$、GR00T 和 LingBot 等路线越来越多地使用独立连续动作专家，直接输出整段连续动作；第三章会解释这一变化为什么发生。
...
[![现代 VLA 动作生成栈](assets/vla-action-stack.svg)](assets/vla-action-stack.svg)

*图 3　动作 Token、压缩 Token 与连续动作专家不是命名差异，而是对精度、速度、多峰性和训练复用的不同权衡。本文归纳，结构依据 [RT-2 Figure 1](https://arxiv.org/abs/2307.15818)、[$\pi_0$ Figure 3](https://www.pi.website/download/pi0.pdf) 与 [$\pi_{0.7}$ Figure 2](https://www.pi.website/download/pi07.pdf)。现代 VLA 越来越倾向让 VLM 管语义、让小型连续专家管动作。*
...
最值得注意的不是 $\pi_0$ 有多少层，而是模型责任已经分开：蓝色 VLM 读取多视角图像和语言，绿色 action expert 读取本体状态 $q_t$ 与噪声，一次生成从 $a_t$ 到 $a_{t+H}$ 的连续动作块。

[![π0 Figure 3：多视角图像与语言进入 VLM，本体状态和噪声进入 action expert，最终输出多个未来连续动作](assets/vla-io-evidence/02-pi0-figure3-action-expert.png)](assets/vla-io-evidence/02-pi0-figure3-action-expert.png)

*图 4　$\pi_0$ 用预训练 VLM 提供语义表征，用较小的 flow-matching action expert 生成连续动作块，并通过本体适配接口控制不同自由度的机器人。裁剪自 [$\pi_0$ 原论文 Figure 3](https://www.pi.website/download/pi0.pdf)，版权归原作者与 Physical Intelligence。*

从输入输出角度看，图中的三个视觉编码器对应不同相机视角，文本 `"fold shirt"` 指定任务，$q_t$ 是机器人当前本体状态。右上角的 $a_t,a_{t+1},\ldots,a_{t+H}$ 才是动作专家的输出；它不是一句“把衣服折好”，而是未来一小段连续控制目标。右侧不同机器人本体可以拥有不同动作维度，说明 VLA 的 Action 必须通过本体专属接口解释，不能脱离机器人定义单独阅读。
...
如果说 RT-2 Figure 1 回答了“动作怎样从 VLM 中输出”，下面这张图回答的是“复杂任务里，谁来决定下一阶段做什么”。从下往上读：高层策略结合当前观测、总任务和历史，生成 `"pick up the knife"` 这样的子任务；世界模型把同一子任务变成多视角视觉子目标；顶层 VLA 再将记忆、文本、目标图像和 metadata 汇合，交给动作专家输出连续动作。

[![π0.7 Figure 2：观察记忆、高层子任务、世界模型视觉子目标和 metadata 共同条件化连续动作专家](assets/vla-io-evidence/03-pi07-figure2-memory-world-model.png)](assets/vla-io-evidence/03-pi07-figure2-memory-world-model.png)

*图 5　$\pi_{0.7}$ 将高层语义策略、MEM 风格观察历史、世界模型生成的视觉子目标和 flow action expert 组合成分层闭环。裁剪自 [$\pi_{0.7}$ 原论文 Figure 2](https://www.pi.website/download/pi07.pdf)，版权归原作者与 Physical Intelligence。*

这张图也明确了 Thinking 的边界：高层策略输出的是可读子任务，世界模型输出的是“期望下一阶段长什么样”，真正驱动机器人运动的仍是右上角动作专家生成的数值动作。Thinking 没有替代 Action，而是在更慢的时间尺度上为 Action 提供条件。
...
> [!note] 证据口径
> 本文优先使用论文、技术报告、模型卡和官方仓库。公司演示只证明“在展示条件下完成过”，不自动证明统计鲁棒性；作者自建 benchmark 只用于说明该论文的内部对照，不与其他论文的成功率直接横比。所有“最新”判断以 2026-07-29 为截止日期。
